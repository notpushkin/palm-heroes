...
void iScenPropsDlg::OnCreateDlg()
{
	...
	for (uint32 xx=0; xx<cnt; ++xx) {
		...
		iPlayerBtn* pPlBtn = new iPlayerBtn(m_pMgr, this, iRect(sx,sy+25,34,34), m_scProps.m_Players[xx].m_Id, m_scProps.m_Players[xx].m_Type, 200+xx, (!m_bReadOnly && (m_scProps.m_Players[xx].m_TypeMask == PTM_HUMAN_ONLY || m_scProps.m_Players[xx].m_TypeMask == PTM_HUMAN_OR_COMPUTER))?(Visible|Enabled):(Visible));
		AddChild(pPlBtn);
