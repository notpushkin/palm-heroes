...
const CTL_CONSTR DEFAULT_CONSTRUCTIONS[CTLT_COUNT][2] = {
	...
	{ CTLCNST_TAVERN, CTLCNST_DWELL_HLFHUT },
...
{
	iPlayer* pOwner = gGame.Map().FindPlayer(pCastle->Owner());
	return pOwner->TavernVisitor() && !m_bRecruited && !pCastle->Visitor() && pOwner->Minerals().Has(pOwner->TavernVisitor()->Proto()->m_Cost) && pOwner->HeroCount() < 4;
}
//////////////////////////////////////////////////////////////////////////
...
void iTreasuryCtlCnst::NewWeek(const iWeekDesc& weekDesk)
{
	if (pCastle->Owner() != PID_NEUTRAL) {
		iPlayer* pPlayer = gGame.Map().FindPlayer(pCastle->Owner());
		pPlayer->Minerals().quant[MINERAL_GOLD] += (pPlayer->Minerals().quant[MINERAL_GOLD] / 10);
	}
}
//////////////////////////////////////////////////////////////////////////
