...
void iVisCnst_Treasury::OnActivate(iHero* pVisitor, bool bActive)
{
	if (Visited(pVisitor)) {
		...
		if (bActive) {
			iDlg_Rewards dlg(&gApp.ViewMgr(),gTextMgr[m_pProto->NameKey()],gTextMgr[TRID_OBJ_TREASURY_DISCOVER],pVisitor,m_rewards);
			dlg.DoModal();
		}
		for (uint32 xx=0; xx<m_rewards.GetSize(); ++xx) pVisitor->ReceiveReward(m_rewards[xx]);
		m_bVisited = true;
	}
	iVisCnst::OnActivate(pVisitor, bActive);
}
