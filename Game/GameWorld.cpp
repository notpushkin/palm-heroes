...
//	pFile->Read(tmpbuff.GetData(),fileSize );
	...
	switch ( mapInfo.m_encType ) {
		...
		case MET_PUBLIC:	cipherKey = PBKEY_PBMAP;
			break;
		case MET_PRIVATE:	cipherKey = gSettings.GetMagicNumber(0);
			break;
#else
		case MET_MAPDES:	cipherKey = PBKEY_MDMAP;
			break;
...
bool iGameWorld::SaveToFile(iFileI* pFile)
{
	...
	dbuff.Write((uint16)MET_PRIVATE);
	// Save timestamp
	SYSTEMTIME st;
	FILETIME ft;
	GetLocalTime(&st);
	...
	// Encrypt
	xxc::cipher_block(PBKEY_COMMON, gSettings.GetMagicNumber(0), (uint8*)cmpBuff.GetData(), cmpBuff.GetSize());
